...
try {
    ...
    $title = $_POST['title'] ?? '';
    $views = $_POST['views'] ?? 0;
    $image_url = $_POST['current_image_url'] ?? ''; // Keep old image if no new upload
    ...
    $stmt = $pdo->prepare("UPDATE songs SET title = ?, image_url = ?, views = ? WHERE id = ?");
    $stmt->execute([$title, $image_url, $views, $id]);
    
    echo json_encode([
        'success' => true,
        'message' => 'อัปเดตเพลงสำเร็จ'
    ]);
} catch (Exception $e) {
    echo json_encode([
        'success' => false,
